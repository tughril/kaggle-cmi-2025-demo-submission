...
def _serialize(data: Any) -> kaggle_evaluation_proto.Payload:
    ...
    if isinstance(data, list):
        return kaggle_evaluation_proto.Payload(
            list_value=kaggle_evaluation_proto.PayloadList(
                payloads=map(_serialize, data)
            )
        )
    elif isinstance(data, tuple):
        return kaggle_evaluation_proto.Payload(
            tuple_value=kaggle_evaluation_proto.PayloadList(
                payloads=map(_serialize, data)
            )
        )
...
def _deserialize(payload: kaggle_evaluation_proto.Payload) -> Any:
    ...
    # Iterables for nested types
        ...
        return {
            ...
            for key, value in payload.dict_value.payload_map.items()
        }
    # Allowlisted special types
    ...
    def _send_with_deadline(
        ...
        if self._made_first_connection:
            try:
                return self.stub.Send(
                    ...
                    timeout=self.endpoint_deadline_seconds,
                )
        ...
        while time.time() - first_call_time < STARTUP_LIMIT_SECONDS:
            for port in GRPC_PORTS:
                ...
                self.stub = kaggle_evaluation_grpc.KaggleEvaluationServiceStub(
                    self.channel
                )
                try:
...
class KaggleEvaluationServiceServicer(
    kaggle_evaluation_grpc.KaggleEvaluationServiceServicer
):
    """
    ...
    def Send(
        ...
        return kaggle_evaluation_proto.KaggleEvaluationResponse(
...
def define_server(*endpoint_listeners: Callable) -> grpc.server:
    ...
    server = grpc.server(
        futures.ThreadPoolExecutor(max_workers=1), options=_GRPC_CHANNEL_OPTIONS
    )
