...
class GatewayRuntimeError(Exception):
    ...
    def __init__(
        self, error_type: GatewayRuntimeErrorType, error_details: Optional[str] = None
    ):
...
class BaseGateway:
    def __init__(
        ...
        self.server = (
            None  # The gateway can have a server but it isn't typically necessary.
        )
    ...
    def validate_prediction_batch(
        ...
        if num_received_rows is None:
            if type(prediction_batch) not in [
                pl.DataFrame,
                pl.Series,
                pd.DataFrame,
                pd.Series,
        ...
        if type(row_ids) not in [pl.DataFrame, pl.Series, pd.DataFrame, pd.Series]:
            raise GatewayRuntimeError(
                ...
                f"Invalid row ID type {type(row_ids)}; expected Polars DataFrame or similar",
            )
        num_expected_rows = len(row_ids)
        if len(row_ids) == 0:
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
                "Missing row IDs for batch",
            )
    ...
    def _standardize_and_validate_paths(
        ...
        if (
            ...
            or type(self.file_share_dir) not in (str, pathlib.Path)
        ):
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
    ...
    def share_files(
        ...
        if self.file_share_dir and not self._shared_a_file:
            if os.path.exists(self.file_share_dir) and (
                not os.path.isdir(self.file_share_dir)
                or len(os.listdir(self.file_share_dir)) > 0
            ):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
                    "`file_share_dir` must be an empty directory.",
                )
        ...
        for in_path, out_path in zip(input_paths, output_paths):
            ...
            if IS_RERUN:
                ...
                try:
                    subprocess.run(
                        f"mount --bind {in_path} {out_path}", shell=True, check=True
                    )
                except Exception:
                    ...
                    os.makedirs(os.path.dirname(out_path), exist_ok=True)
                    subprocess.run(
                        f"cp -r {in_path} {out_path}", shell=True, check=True
                    )
            else:
    ...
    def write_submission(
        self,
        predictions,
        row_ids: List[Union[pl.Series, pl.DataFrame, pd.Series, pd.DataFrame]],
    ) -> None:
        ...
        if isinstance(predictions, list):
            if isinstance(predictions[0], pd.DataFrame):
                ...
                if self.target_column_name is None:
                    raise GatewayRuntimeError(
                        GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
                        "`target_column_name` must be set in order to use scalar value predictions.",
                    )
                predictions = pl.DataFrame(
                    data={
                        row_ids.columns[0]: row_ids,
    ...
    def handle_server_error(self, exception: Exception, endpoint: str) -> None:
        ...
        if isinstance(exception, gaierror) or (
            isinstance(exception, RuntimeError)
            and "Failed to connect to server after waiting" in exception_str
        ):
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.SERVER_NEVER_STARTED
            ) from None
